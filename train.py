...
def train(args):
    ...
    result_dir_train = os.path.join(result_dir)
    ...
    if mode == 'train':
        ...
        # loader_val = DataLoader(dataset_val, batch_size=batch_size, shuffle=True, num_workers=8) # DateLoader는 Dataset class의 instance를 인자로 받아 데이터를 batch_size만큼씩 
    ...
    ## 네트워크 생성하기
    if network == "pix2pix":
        netG = Pix2Pix_generator(in_channels=nch,out_channels=nch,nker=nker,norm=norm).to(device) # G는 input과 동일한 채널의 아웃풋 이미를 생성
    ...
    if mode == 'train':
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            #     plt.imsave(os.path.join(result_dir_train, 'png', '%04d_input.png_%d' % (id,epoch)), input[0].squeeze(), cmap=cmap)
            #     plt.imsave(os.path.join(result_dir_train, 'png', '%04d_label.png_%d' % (id,epoch)), label[0].squeeze(), cmap=cmap)
            #     plt.imsave(os.path.join(result_dir_train, 'png', '%04d_output.png_%d' % (id,epoch)), output[0].squeeze(), cmap=cmap)
            ...
            if epoch % 500 == 0: # epoch 20번마다 네트워크 checkpoint 저장
